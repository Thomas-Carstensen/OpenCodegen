...
describe('generateClients', () => {
  ...
  test('generates method from operationId', () => {
    ...
    expect(content).toContain('async listPets()');
    expect(content).toContain('async createPet(');
  });
  ...
  test('handles path parameters', () => {
    ...
    expect(content).toContain('async getPet(petId: number)');
  ...
  test('handles request body', () => {
    ...
    expect(content).toContain('async createPet(body: Pet)');
